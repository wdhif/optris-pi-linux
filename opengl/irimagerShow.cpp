...
void drawMeasurementInfo(Obvious2D* viewer, ImageBuilder* iBuilder, unsigned char* dst, unsigned int w, unsigned int h, unsigned int x, unsigned int y, float value, unsigned char rgba[4], unsigned char rgbaBG[4])
{
...
void cbSnapshot()
{
  ...
  cout << "Minimum temperature: " << _iBuilder.getIsothermalMin() << ", Maximum temperature: " << _iBuilder.getIsothermalMax() << endl;
...
void* displayWorker(void* arg)
{
  ...
  Obvious2D viewer(w, h, "Optris Imager Example");
  ...
  while(viewer.isAlive() && !_shutdown)
  {
    ...
    else
    {
      ...
      unsigned char black[4]   = {0, 0, 0, 255};
      drawMeasurementInfo(&viewer, &_iBuilder, bufferThermal, w, h, (minRegion.u1+minRegion.u2)/2, (minRegion.v1+minRegion.v2)/2, minRegion.t, rgba, white);
      rgba[0] = 255;
      rgba[2] = 0;
      drawMeasurementInfo(&viewer, &_iBuilder, bufferThermal, w, h, (maxRegion.u1+maxRegion.u2)/2, (maxRegion.v1+maxRegion.v2)/2, maxRegion.t, rgba, white);
...
int main (int argc, char* argv[])
{
  ...
  cout << "Connected camera, serial: " << _imager->getSerial() << ", HW(Rev.): " << _imager->getHWRevision() << ", FW(Rev.): " << _imager->getFWRevision() << endl;
